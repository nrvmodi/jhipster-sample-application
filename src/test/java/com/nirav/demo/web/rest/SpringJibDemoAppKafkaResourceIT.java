package com.nirav.demo.web.rest;
...
import com.nirav.demo.config.KafkaProperties;
...
class SpringJibDemoAppKafkaResourceIT {
    ...
    void setup() {
        ...
        SpringJibDemoAppKafkaResource kafkaResource = new SpringJibDemoAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/spring-jib-demo-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/spring-jib-demo-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
